"""temporal-order-3bit — Hochreiter & Schmidhuber 1997 (NC) Experiment 6a.

Four-class temporal-order task. A sequence carries two "important" symbols
drawn from {X, Y} embedded at unknown positions; everything else is a random
distractor in {a, b, c, d}. The target class encodes the order of the two
important symbols: XX, XY, YX, YY. The distance between the two markers is
roughly two thirds of the sequence, so a vanilla RNN cannot bridge the gap
through the distractor stream. An LSTM with two memory cells solves it.

This file holds the entire pipeline: dataset generator, vanilla LSTM with
BPTT (no forget gate, matching the 1997 NC paper), vanilla recurrent net
baseline, training loops for both, and a CLI that reproduces the headline
result. Pure numpy + matplotlib.
"""
...
# Class labels: (first, second) -> class id
#   0 = XX, 1 = XY, 2 = YX, 3 = YY
CLASS_NAMES = ("XX", "XY", "YX", "YY")


def _label(first: int, second: int) -> int:
    a = 0 if first == X_TOK else 1
    b = 0 if second == X_TOK else 1
    return 2 * a + b


def make_sequence(rng: np.random.Generator, *, T: int = 50,
                  t1_range=(3, 12), t2_range=(25, 40)) -> tuple[np.ndarray, int]:
    """Build one (one-hot sequence, class) pair.

    Position 0 is B, position T-1 is E. Two slots t1, t2 carry independently
    drawn symbols from {X, Y}; every other interior position is a random
    distractor from {a, b, c, d}.
    """
...
def init_lstm(rng: np.random.Generator, *, hidden: int = 4,
              vocab: int = VOCAB, n_classes: int = 4) -> LSTMParams:
    s = 0.1
...
def init_rnn(rng: np.random.Generator, *, hidden: int = 4,
             vocab: int = VOCAB, n_classes: int = 4) -> RNNParams:
    s = 0.1
...
def train_lstm(rng: np.random.Generator, *, T: int, hidden: int, lr: float,
    ...
    for step in range(1, n_steps + 1):
        ...
        if step == 1 or step % eval_every == 0:
            ...
            if record_hidden:
                # snapshot hidden trace for one fixed batch of recorded sequences.
...
def _gradcheck(seed: int = 0, T: int = 8, B: int = 3, H: int = 3) -> float:
    ...
    p = init_lstm(rng, hidden=H, vocab=VOCAB, n_classes=4)
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Train LSTM and RNN baseline on temporal-order-3bit.")
...
def main() -> None:
    ...
    # One fixed mini-batch of representative sequences for the GIF (one per class if possible).
    ...
    for i, row in enumerate(cm):
        print(f"  {CLASS_NAMES[i]:>2s} -> {row.tolist()}")
